	...
	//			to use them elsewhere in the app like this way
		...
		object EstimateSession {
			private const val PREFS_NAME = "EstimatePrefs"
			private const val KEY_CURRENT_ESTIMATE = "current_estimate"

			fun setCurrentEstimate(context: Context, estimateId: Int) {
				context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
					.edit()
					.putInt(KEY_CURRENT_ESTIMATE, estimateId)
					.apply()
			}

			fun getCurrentEstimate(context: Context): Int {
				return context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
					.getInt(KEY_CURRENT_ESTIMATE, -1)
			}

			fun clearCurrentEstimate(context: Context) {
				context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
					.edit()
					.remove(KEY_CURRENT_ESTIMATE)
					.apply()
			}
		}
		...
		//use for the previews
		fun pdfToImage(pdfFile: File, pageIndex: Int = 0): Bitmap? {
			try {
		...
		//use of shared preferences to store the cached versions
